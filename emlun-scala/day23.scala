object Day23 extends App {
  ...
  def solveA(state: MachineState) = {
    val states = Iterator.iterate(state)({ state: MachineState => state.next(state)._1 }).takeWhile({ !_.isFinished })
    states.count({ state => state.program(state.eip) match {
      case m: Mul => true
      case _ => false
    } })
  }
  ...
  def solveB(program: Vector[Instruction]): Long = {
    ...
    solveB((MachineState(programB, Map.empty), MachineState(programB, Map.empty)))
  }
