...
const getNumbersForRow = (rowString) => {
...
const calculateRow = (rowString) => {
    const sortedRow = getNumbersForRow(rowString)
...
const calculateRowForEvenNumbers = (rowString) => {
    return getNumbersForRow(rowString)
        .map(number => parseInt(number))
        .map((number, _, array) => {
            const matchingDevideNumber = array.find(num => (num % number === 0 && num !== number) ? num : void 0)
            return matchingDevideNumber ? [matchingDevideNumber, number] : []
        })
...
const calculateChecksum = (input) => {
    return getRows(input)
        .map(calculateRowForEvenNumbers)
        .reduce((a, b) => a + b, 0)
}
