...
#         if line and no_duplicates(line.strip().split(' '))
#     ]
...
def no_duplicates2(words):
    return len(words) == len(set(''.join(sorted(word)).strip() for word in words))


def solve2(input):
    unique_lines = [
        line for line in input.split('\n')
        if line and no_duplicates2(line.strip().split(' '))
    ]
    return len(unique_lines)
