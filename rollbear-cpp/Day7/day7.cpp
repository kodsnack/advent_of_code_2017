...
using tree = std::map<std::string, programinfo>;

void parse_children(const std::string& parent, std::istream& is, tree& p)
{
    auto& here = p[parent];
    std::string arrow;
    is >> arrow;
    ...
    while (is >> namestr)
    {
        auto name = namestr.substr(0,namestr.find(','));
        auto& child = p[name];
        child.parent = parent;
        here.children.insert(name);
    }
}

void parse_programdata(const std::string& s, tree& p)
{
    std::string name;
    size_t weight;
    char c;
    ...
    is >> name >> c >> weight >> c;
    p[name].weight = weight;
    parse_children(name, is, p);
}

tree parse_tree(const std::vector<std::string>& description)
{
    tree p;
    for (auto& line : description)
    {
        parse_programdata(line, p);
    }
    return p;
}

auto root_finder = [](auto& a) { return a.second.parent == "";};

auto find_node = [](const tree &p, auto pred)
{
    return *std::find_if(std::begin(p), std::end(p),
                         pred);
};

std::set<std::string> find_children(const std::string& parent, const tree& p)
{
    return p.at(parent).children;
}

std::pair<const std::string, programinfo>* find_imballanced_node(tree& p, const std::string& name)
{
    auto & node = p[name];
    std::map<size_t, std::vector<std::string>> weights;
    for (auto& c : node.children)
    {
        auto w = find_imballanced_node(p, c);
        if (w) return w;
        weights[p[c].cumulative_weight].push_back(c);
    }
    if (weights.size() == 0)
    {
        node.cumulative_weight = node.weight;
        return nullptr;
    }
    if (weights.size() == 1)
    {
        node.cumulative_weight = weights.begin()->first * weights.begin()->second.size() + node.weight;
        return nullptr;
    }
    auto i = weights.begin();
    auto minority_weight = i->second.size() == 1U ? i->first : std::next(i)->first;
    return &*p.find(weights[minority_weight].front());
}

size_t balance_node(const tree& p, const std::string& node_name)
{
    auto& node = p.at(node_name);
    auto& parent = p.at(node.parent);
    for (auto& c : parent.children)
    {
        if (c != node_name)
        {
            auto sibling_weight = p.at(c).cumulative_weight;
            auto node_weight = node.cumulative_weight;
            return node.weight + sibling_weight - node_weight;
        }
    }
    return 0U;
}

TEST_CASE("children can be parsed")
{
    std::istringstream is{" -> foo, bar, baz"};
    tree p;
    parse_children("-",is, p);
    REQUIRE(p.size() == 4U);
    REQUIRE(p["foo"].parent == "-");
    REQUIRE(p["bar"].parent == "-");
    REQUIRE(p["baz"].parent == "-");
}

TEST_CASE("a tree node can be parsed")
{
    tree p;
    parse_programdata("top (32) -> foo, bar, baz", p);
    REQUIRE(p.size() == 4U);
    REQUIRE(p["top"].parent == "");
    REQUIRE(p["top"].weight == 32U);
    REQUIRE(p["foo"].parent == "top");
    REQUIRE(p["bar"].parent == "top");
    REQUIRE(p["baz"].parent == "top");
}

TEST_CASE("the root can be found")
{
    ...
    auto root = find_node(parse_tree(data), root_finder);
    REQUIRE(root.first == "tknk");
}

TEST_CASE("the children of a node can be found")
{
    auto t = parse_tree({"a (5) -> foo, bar, baz"});
    auto c = find_children("a", t);
    REQUIRE(c == (std::set{"foo"s, "bar"s, "baz"s}));
}

TEST_CASE("an imbalanced node can be found")
{
    std::vector<std::string> data {
            "pbga (66)",
            "xhth (57)",
    ...
    auto p = parse_tree(data);
    auto n = find_imballanced_node(p, "tknk");
    REQUIRE(n->first == "ugml");
    REQUIRE(balance_node(p, "ugml") == 60U);
}
int main(int argc, char *argv[])
{
    ...
    if (result == 0)
    {
        ...
        auto t = parse_tree(data);
        auto root = find_node(t, root_finder);
        std::cout << "one=" << root.first << '\n';
        auto n = find_imballanced_node(t, root.first);
        std::cout << "two=" << balance_node(t, n->first) <<'\n';
    }
    return result;
}
