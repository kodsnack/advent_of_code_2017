...
std::map<std::string, std::function<bool(int,int)>> const operators
        {
                {"==", std::equal_to<>{}},
                {"!=", std::not_equal_to<>{}},
                {"<", std::less<>{}},
...
void parse_and_execute(const std::string& s, registers& r)
{
    ...
    std::string cond_op;
    int gate;
    is >> reg >> op >> delta >> cond >> cond_reg >> cond_op >> gate;
    assert(cond == "if");
    auto& reg_value = r[reg];
    if (operators.at(cond_op)(r[cond_reg], gate))
    {
        if (op == "inc")
        {
            reg_value += delta;
...
auto largest_register(const registers& r)
{
    auto i = std::max_element(std::begin(r), std::end(r),
                              [](auto& x, auto& y) { return x.second < y.second;});
    return i->first;
}

int execute_program(std::vector<std::string> instructions, registers& r)
{
    int largest_value = std::numeric_limits<int>::min();
    for (auto& i : instructions)
    {
        parse_and_execute(i, r);
        auto largest_reg = largest_register(r);
        if (r[largest_reg] > largest_value)
        {
            largest_value = r[largest_reg];
        }
    }
...
TEST_CASE("instructions can be executed")
{
    registers r;
    parse_and_execute("b inc 5 if a > 1", r);
    REQUIRE(r["b"] == 0);
    REQUIRE(r["a"] == 0);
    parse_and_execute("a inc 1 if b < 5", r);
    REQUIRE(r["a"] == 1);
    REQUIRE(r["b"] == 0);
    parse_and_execute("c dec -10 if a >= 1", r);
    REQUIRE(r["c"] == 10);
    REQUIRE(r["a"] == 1);
    parse_and_execute("c inc -20 if c == 10", r);
    REQUIRE(r["c"] == -10);
}

TEST_CASE("largest register can be found")
{
    registers r{
            {"a", 5},
            {"b", 2},
...
int main(int argc, char *argv[])
{
    ...
    if (result == 0)
    {
        ...
        registers r;
        auto largest_value = execute_program(program, r);
        auto largest = largest_register(r);
        std::cout << "one=" << r[largest] << '\n';
        std::cout << "two=" << largest_value << '\n';
    }
    return result;
}
