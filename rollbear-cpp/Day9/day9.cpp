...
{
public:
    lexer(std::string s) : data(std::move(s)), pos(data.begin()) {}
    char next_char()
    {
        if (pos == data.end())
        {
    ...
    int cancelled_chars() const { return cancelled;}
private:
...
int parse_groups(lexer& lex, int group_score = 1, int group_delta = 0)
{
    int sum = 0;
    for (;;) {
        auto c = lex.next_char();
        if (c == '\0')
        {
            return sum;
        }
        if (c == '{') {
            sum += parse_groups(lex, group_score + group_delta, group_delta);
        }
        if (c == '}') {
            return sum + group_score;
        }
    }
}

TEST_CASE("lexer::next_char gives next non-garbage character")
{
    REQUIRE(lexer{"<>a"}.next_char() == 'a');
    REQUIRE(lexer{"<<<<>a"}.next_char() == 'a');
    REQUIRE(lexer{"<{!>}>a"}.next_char() == 'a');
    REQUIRE(lexer{"<!!>a"}.next_char() == 'a');
    REQUIRE(lexer{"<!!!>>a"}.next_char() == 'a');
    REQUIRE(lexer{"<{o\"i!a,<{i<a>_"}.next_char() == '_');
}

TEST_CASE("parse_groups returns number of groups in stream")
{
    {
        lexer stream{"{}"};
        REQUIRE(parse_groups(stream) == 1);
    }
    {
        lexer stream{"{{{}}}"};
        REQUIRE(parse_groups(stream) == 3);
    }
    {
        lexer stream{"{{},{}}"};
        REQUIRE(parse_groups(stream) == 3);
    ...
    {
        lexer stream{"{<a>,<a>,<a>,<a>}"};
        REQUIRE(parse_groups(stream) == 1);
    }
    {
        lexer stream{"{{<a>},{<a>},{<a>},{<a>}}"};
        REQUIRE(parse_groups(stream) == 5);
    }
    {
        lexer stream{"{{<!>},{<!>},{<!>},{<a>}}"};
        REQUIRE(parse_groups(stream) == 2);
    }
}

TEST_CASE("parse_groups can give incremental scores for depth")
{
    {
        lexer stream{"{}"};
        REQUIRE(parse_groups(stream, 0, 1) == 1);
    }
    {
        lexer stream{"{{{}}}"};
        REQUIRE(parse_groups(stream, 0, 1) == 6);
    }
    {
        lexer stream{"{{},{}}"};
        REQUIRE(parse_groups(stream, 0, 1) == 5);
    }
    {
        lexer stream{"{{{},{},{{}}}}"};
        REQUIRE(parse_groups(stream, 0, 1) == 16);
    }
    {
        lexer stream{"{<a>,<a>,<a>,<a>}"};
        REQUIRE(parse_groups(stream, 0, 1) == 1);
    ...
    {
        lexer stream{"{{<ab>},{<ab>},{<ab>},{<ab>}}"};
        REQUIRE(parse_groups(stream, 0, 1) == 9);
    }
    {
        lexer stream{"{{<!!>},{<!!>},{<!!>},{<!!>}}"};
        REQUIRE(parse_groups(stream, 0, 1) == 9);
    ...
    {
        lexer stream{"{{<a!>},{<a!>},{<a!>},{<ab>}}"};
        REQUIRE(parse_groups(stream, 0, 1) == 3);
    }
}

TEST_CASE("lexer keeps count of cancelled characters")
{
    {
        lexer stream{"<>"};
        while (stream.next_char());
        REQUIRE(stream.cancelled_chars() == 0);
    }
    {
        lexer stream{"<random characters>"};
        while (stream.next_char());
        REQUIRE(stream.cancelled_chars() == 17);
    }
    {
        lexer stream{"<<<<>"};
        while (stream.next_char());
        REQUIRE(stream.cancelled_chars() == 3);
    }
    {
        lexer stream{"<{!>}>"};
        while (stream.next_char());
        REQUIRE(stream.cancelled_chars() == 2);
    }
    {
        lexer stream{"<!!>"};
        while (stream.next_char());
    ...
    {
        lexer stream{"<{o\"i!a,<{i<a>"};
        while (stream.next_char());
        REQUIRE(stream.cancelled_chars() == 10);
    }
}
