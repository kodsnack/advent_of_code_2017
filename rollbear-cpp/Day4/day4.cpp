...
Container<std::string> split(const std::string& v)
{
    Container<std::string> rv;
...
template <typename T>
size_t count_unique_elements(std::vector<T> v)
{
    std::sort(std::begin(v), std::end(v));
    auto end = std::unique(std::begin(v), std::end(v));
    return std::distance(std::begin(v), end);
}


template <typename Predicate>
size_t count_valid_passwords(std::vector<std::string> passwords, Predicate p)
{
    return std::count_if(std::begin(passwords),
                         std::end(passwords),
                         p);
}

bool all_words_are_unique(const std::string s)
{
    auto v = split<std::vector>(s);
    return v.size() == count_unique_elements(v);
}
...
bool no_words_are_anagrams(const std::string& s)
{
    auto v = split<std::vector>(s);
    std::vector<std::string> sorted_words;
    std::transform(std::begin(v), std::end(v),
                   std::back_inserter(sorted_words),
                   sorted<std::string>);
    return count_unique_elements(sorted_words) == sorted_words.size();
}

TEST_CASE("split string into vector of individual words")
{
    auto v = split<std::vector>("aa bb   cc d e   fff ");
    std::vector expected{"aa"s, "bb"s, "cc"s, "d"s, "e"s, "fff"s};
    REQUIRE(v == expected);
    REQUIRE(split<std::vector>("").empty());
}

TEST_CASE("count_unique_elements returns number of unique elements in a vector")
{
    REQUIRE(count_unique_elements(std::vector{1,2,3,2,1}) == 3U);
}

TEST_CASE("valid passwords have no repeated words")
{
    REQUIRE(count_valid_passwords(std::vector{"aa bb cc dd ee"s}, all_words_are_unique) == 1U);
    REQUIRE(count_valid_passwords(std::vector{"aa bb cc dd aa"s}, all_words_are_unique) == 0U);
    REQUIRE(count_valid_passwords(std::vector{"aa bb cc dd aaa"s}, all_words_are_unique) == 1U);
}

TEST_CASE("anagram words are detected")
{
    REQUIRE(are_anagrams("foo", "oof"));
    REQUIRE(are_anagrams("foo", "ofo"));
    REQUIRE_FALSE(are_anagrams("foo", "fof"));
    REQUIRE_FALSE(are_anagrams("foo", "foof"));
}

TEST_CASE("passwords with anagrams are detected")
{
    REQUIRE(count_valid_passwords(std::vector{"aa bb aaa"s}, no_words_are_anagrams) == 1U);
    REQUIRE(count_valid_passwords(std::vector{"ab ba"s}, no_words_are_anagrams) == 0U);
}
...
int main(int argc, char *argv[])
{
    ...
    if (result == 0)
    {
        ...
        std::cout << "one=" << count_valid_passwords(input, all_words_are_unique) << '\n';
        std::cout << "two=" << count_valid_passwords(input, no_words_are_anagrams) << '\n';
    }
    return result;
}
