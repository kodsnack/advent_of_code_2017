...
using village = std::map<int, node>;

std::pair<int, node> parse_connection(const std::string& s)
{
    ...
    do
    {
        int peer_id;
        is >> peer_id;
        n.peers.insert(peer_id);
    } while (is >> c);
    return {id, n};
}

village populate_village(const std::vector<std::string>& in)
{
    village programs;
    for (auto& s : in)
    {
        auto [id, n] = parse_connection(s);
        programs[id] = n;
    }
    return programs;
}

void calculate_distance_to(village& v, int target)
{
    int steps = 0;
    std::vector<int> nodes{target};
    while (!nodes.empty())
    {
        ...
        {
            auto& n = v[id];
            for (auto peer_id : n.peers)
            {
                auto& peer = v[peer_id];
                if (peer.distance == 0)
                {
                    peer.distance = steps + 1;
...
std::vector<int> nodes_that_can_reach_target(const village& v)
{
    std::vector<int> rv;
    for (auto& n : v)
    {
        if (n.second.distance > 0)
        {
            rv.push_back(n.first);
        }
    }
    return rv;
}

int find_number_of_groups(village v)
{
    int rv = 0;
    while (!v.empty())
    {
        int target = v.begin()->first;
        calculate_distance_to(v, target);
        auto members = nodes_that_can_reach_target(v);
        for (auto id : members)
        {
            v.erase(id);
        }
...
TEST_CASE("node connections can be parsed")
{
    {
        auto [id, n] = parse_connection("0 <-> 2");
        REQUIRE(id == 0);
        REQUIRE(n.peers == (std::set{2}));
    }
    {
        auto [id, n] = parse_connection("2 <-> 0,3,4");
        REQUIRE(id == 2);
        REQUIRE(n.peers == (std::set{0,3,4}));
    }
}

TEST_CASE("a village of connections can be populated")
{
    auto p = populate_village(
            {
                    "0 <-> 2",
                    "1 <-> 1",
                    "2 <-> 0, 3, 4",
    ...
    REQUIRE(p[2].peers == (std::set{0,3,4}));
    REQUIRE(p[0].peers == (std::set{2}));
}

TEST_CASE("distance to zero can be calculated")
{
    auto p = populate_village(
            {
                    "0 <-> 2",
    ...
    calculate_distance_to(p, 0);
    REQUIRE(p[2].distance == 1);
    REQUIRE(p[5].distance == 4);
}

TEST_CASE("nodes that can reach zero is calculated")
{
    auto p = populate_village(
            {
                    "0 <-> 2",
    ...
    calculate_distance_to(p, 0);
    auto v = nodes_that_can_reach_target(p);
    REQUIRE(v == (std::vector({0,2,3,4,5,6})));
}

TEST_CASE("number of groups can be calculated")
{
    auto p = populate_village(
            {
                    "0 <-> 2",
    ...
    REQUIRE(find_number_of_groups(p) == 2);
}
int main(int argc, char *argv[])
{
    ...
    if (result == 0)
    {
        ...
        auto p = populate_village(connections);
        calculate_distance_to(p, 0);
        auto v = nodes_that_can_reach_target(p);
        std::cout << "one=" << v.size() << '\n';
        std::cout << "two=" << find_number_of_groups(p) << '\n';
    }
    return result;
}
